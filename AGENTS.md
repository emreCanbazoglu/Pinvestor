...
Skill file registered at `~/.Codex/skills/game-element-designer.md`.
...
## Parallel Implementation Workflow (Codex Agents + Codex Review)

Specs are implemented by spawning parallel OpenAI Codex agents via CLI, one per spec (or per phase). Codex reviews each PR, agents iterate on feedback, and all approved PRs are merged to `main`.
...
5. **If a feature is already substantially implemented**, the agent must:
   ...
   - Flag the discovery to Codex review so the spec's task list can be updated
...
### Step 3 — Codex Reviews the PR

Codex reviews each PR against:
...
```sh
# Codex reviews a PR by number
gh pr view <PR_NUMBER> --json headRefName,body,files | Codex "Review this PR against its spec tasks.md and constitution rules. List any violations or missing tasks."
```
...
Once Codex approves a PR (no unresolved comments, all required tasks checked off):
...
- Do not edit `AGENTS.md`, `specs/`, or `docs/` — design docs are read-only for agents
